...
sealed class Type(val constraints: List<ConstraintExpr> = listOf()) {
  ...
  class Reference(
    ...
    override fun withConstraints(constraints: List<ConstraintExpr>): Type =
      Reference(ctx, typeArguments, constraints)

    override fun withTypeArguments(argument1: Type): Class =
      Reference(ctx, listOf(argument1), constraints)

    override fun withTypeArguments(argument1: Type, argument2: Type): Class =
      Reference(ctx, listOf(argument1, argument2), constraints)
        ...
        visitor.exactName != null -> {
          ...
          walkCandidates(referent, base, context) { type, properties ->
            if (type is Class && type.classEquals(base.nullType)) {
              candidates.add(
                PklReferenceQualifiedAccessProxyImpl.ClassProperty(
                  ctx.project,
                  visitor.exactName!!,
                  base.nullType,
                )
          ...
          when {
            isInvalid -> true
            isUnknown -> visit(visitor.exactName!!, PklFakeUnknownTypeImpl(ctx.project), candidates)
            !candidates.isEmpty() ->
              visit(
                visitor.exactName!!,
    ...
    companion object {
      fun walkCandidates(
        ...
        base: PklBaseModule,
        context: PklProject?,
        visit: (Type, List<PklClassProperty>) -> Boolean,
      ): Boolean =
        when (root) {
          is Alias -> {
